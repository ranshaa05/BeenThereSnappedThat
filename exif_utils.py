...
def get_gps_info(exif_data: dict):
    ...
    if gps_info := exif_data.get('GPSInfo'):
        return {GPSTAGS.get(k, k): v for k, v in gps_info.items()}
    return None
...
def dms_to_decimal(dms, ref: str):
    ...
    if not dms or len(dms) != 3:
        return None
    try:
        deg, min_, sec = map(_rational_to_float, dms)
